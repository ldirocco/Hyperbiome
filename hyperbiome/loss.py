...
class HypProxyAnchor(nn.Module):
    def __init__(self, nb_classes, sz_embed, c=0.1,  alpha=32, clip_r=2.3, riemannian=True):
        super().__init__()
        ...
        self.projector = HypProjector(c=0.1, riemannian=riemannian, clip_r=clip_r)
...
class HierarchicalProxyAnchor(nn.Module):
    ...
    def get_genera_proxies(self):
        with torch.no_grad():
            return self.projector(self.genus_proxies_tan)


class HierarchicalProxyAnchor_Euclidean(nn.Module):
    def __init__(self, n_genus, n_species, sz_embed, metadata_path):
        super().__init__()
        self.n_genus = n_genus
        self.n_species = n_species
    ...
    def forward(self, X, T_species):
        # Proxies
        device = X.device

        genus_proxies = self.genus_proxies_tan.to(device=device)
        species_proxies = self.species_proxies_tan.to(device=device)

        # ---- LOSS SPECIES ----

        dist_species = torch.cdist(X, species_proxies, p=2)  # Euclidean Distance
        ...
        # Calcolo la distanza tra il sottoinsieme di proxies delle species e le proxy dei genera
        dist_genus = torch.cdist(species_proxies_unique, genus_proxies, p=2)
    ...
    def get_species_proxies(self):
        with torch.no_grad():
            return self.species_proxies_tan

    def get_genera_proxies(self):
        with torch.no_grad():
            return self.genus_proxies_tan